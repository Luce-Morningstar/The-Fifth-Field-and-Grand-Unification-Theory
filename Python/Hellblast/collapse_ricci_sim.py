...
# 2b. HELLBLAST alm2map reconstruction (real harmonic inversion)
...
def generate_angles(npix):
    ...
    if npix != npix_calc:
        print(f"[⚠️ Warning] Pixel mismatch: got {npix}, expected {npix_calc}. Auto-correcting to nside={nside}...")
        npix = npix_calc
    i = cp.arange(npix)
    z = 1 - 2 * (i + 0.5) / npix
    theta = cp.arccos(z)
...
# Step 1: Ricci Field Evolution
frames = evolve_fields(A_field, B_field, steps=24)

# Step 2: Observer Tensor and Temporal Ricci-Ani Inflation
obs_center = (grid_size // 2, grid_size // 2)
obs_density = cp.zeros((grid_size, grid_size))
for i in range(grid_size):
    for j in range(grid_size):
        dist = cp.sqrt((i - obs_center[0])**2 + (j - obs_center[1])**2)
        obs_density[i, j] = cp.exp(-0.01 * dist**2)
obs_density /= obs_density.max()

ricci_ani_frames = []
for frame in frames:
    f_gpu = cp.asarray(frame)
    warped = f_gpu * obs_density
    ricci_ani_frames.append(warped.get())

ani_cube = np.stack(ricci_ani_frames)
save_fits(ani_cube, "Ricci_Ani_Timecube.fits")

# Step 3: Ricci Scar Injection
final_Ricci = cp.asarray(frames[-1])
ricci_volume = inject_ricci_scar(final_Ricci)

# Step 4: Spherical Projection via HELLBLAST
flat_field = final_Ricci.flatten()
npix = flat_field.size
theta, phi = generate_angles(npix)
flat_field = flat_field[:theta.shape[0]]
alm = map2alm(flat_field, theta, phi, lmax=10)

try:
    mollview_gpu(flat_field, nside=int(np.sqrt(npix / 12)), title="Ricci Harmonic Input", output_file="Ricci_Mollweide.png")
except ValueError as e:
    print(f"[Mollview Warning] {e}")

# Step 5: Summary and FITS Exports
